...
export default function PaymentPage() {
  ...
  const handlePaymentSubmit = async (_paymentData: {
    cardNumber: string;
    ...
    try {
      ...
      const message = error?.response?.data?.error || 'El pago falló. Por favor, inténtalo de nuevo.';
      toast.error(message);
      setSubmitting(false);
    }
  };
            ...
            <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl border-4 border-yellow-500 p-8 relative overflow-hidden">
              ...
              <div className="flex gap-6 mb-8">
                {moviePoster && (
                  <div className="flex-shrink-0">
                    <img
                      src={moviePoster}
                      alt={movieName}
                      className="w-32 h-48 object-cover rounded-xl border-4 border-yellow-500 shadow-2xl"
                    />
                  </div>
                )}
                <div className="flex-1">
                  ...
                  <div className="space-y-2 text-gray-300">
                    <p>{new Date(sessionDateTime).toLocaleDateString('es-ES', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
                    <p>{new Date(sessionDateTime).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-800/50 rounded-xl p-6 border-2 border-gray-700 space-y-4">
                ...
                <div className="flex justify-between items-center pb-3 border-b border-gray-700">
                  ...
                  <span className="text-white font-black text-xl">${seatCount > 0 ? (totalAmount / seatCount).toFixed(2) : '0.00'}</span>
                </div>
