...
export default function CustomerRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = error?.response?.data?.error || 'No se pudo registrar la cuenta';
      toast.error(message);
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black py-16 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">Regístrate para comprar entradas, ver tu historial y valorar las películas que viste.</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-900 to-black border-4 border-emerald-500 rounded-2xl p-8 shadow-2xl">
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-yellow-400 font-black text-sm tracking-wider mb-2">NOMBRE</label>
                <input
                  type="text"
                  required
                  value={formData.Name}
              ...
              <div>
                <label className="block text-yellow-400 font-black text-sm tracking-wider mb-2">APELLIDO</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-yellow-400 font-black text-sm tracking-wider mb-2">CORREO</label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className="block text-yellow-400 font-black text-sm tracking-wider mb-2">TELÉFONO</label>
                <input
                  type="tel"
                  required
              ...
              <div>
                <label className="block text-yellow-400 font-black text-sm tracking-wider mb-2">CONTRASEÑA</label>
                <input
                  type="password"
                  required
                  minLength={6}
              ...
              <div>
                <label className="block text-yellow-400 font-black text-sm tracking-wider mb-2">CONFIRMAR CONTRASEÑA</label>
                <input
                  type="password"
                  required
                  minLength={6}
            ...
            <p className="mt-6 text-sm text-gray-400 text-center">
              ¿Ya tienes cuenta?{' '}
              <Link href={`/account/login?redirect=${encodeURIComponent(redirectTo)}`} className="text-emerald-400 font-bold hover:text-emerald-300">
                Inicia sesión
              </Link>
            </p>
          </div>
