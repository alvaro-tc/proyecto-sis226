...
const { createAuthToken, getUserPayload, requireAuth } = require('../middleware/auth');
...
router.post('/register/customer', async (req, res) => {
  try {
    ...
    const usernameBase = normalizeIdentity(`${Name}.${Surname}`.replace(/\s+/g, ''));
...
router.patch('/me/profile', requireAuth, async (req, res) => {
  try {
    ...
    const updatedUser = await User.findById(req.user._id).populate('CustomerID');
    res.json({
